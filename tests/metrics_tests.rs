...
fn test_event_metrics() {
    ...
    let events_by_type =
        MockCounter::new("events_by_type_total", "Events by type").with_label("type", "network");
...
fn test_network_metrics() {
    ...
    let network_connections = MockGauge::new(
        "network_connections",
    ...
    let network_bytes_received =
        MockCounter::new("network_bytes_received_total", "Total bytes received");
...
fn test_security_metrics() {
    ...
    let blocked_connections =
        MockCounter::new("blocked_connections_total", "Total blocked connections");
    let authentication_failures =
        MockCounter::new("authentication_failures_total", "Authentication failures");
...
impl MockMetricRegistry {
    ...
    fn register_histogram(&self, histogram: MockHistogram) {
        self.histograms
            .lock()
            .unwrap()
            .insert(histogram.name.clone(), histogram);
    }
...
fn test_metric_performance() {
    ...
    println!(
        "Performed {} metric operations in {:?} ({:.2} ops/sec)",
    ...
    assert!(
        ...
        "Metric performance too low: {:.2} ops/sec",
        ops_per_second
    );
