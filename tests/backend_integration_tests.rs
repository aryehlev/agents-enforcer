...
use crate::{require_ebpf, require_linux, require_macos, require_windows, require_privileges};
...
fn test_backend_registry() {
    ...
    println!("Linux backend available: {}", config.platform.run_linux_tests);
    println!("macOS backend available: {}", config.platform.run_macos_tests);
    println!("Windows backend available: {}", config.platform.run_windows_tests);
    println!("eBPF programs available: {}", config.platform.ebpf_available);
}
...
fn test_backend_lifecycle() {
    ...
    // Test initial state
    assert!(!mock_backend.is_initialized(), "Backend should not be initialized initially");
    assert_eq!(mock_backend.name(), "test_backend");
    assert_eq!(mock_backend.backend_type(), "mock");
    
    // Test initialization
    mock_backend.initialize().expect("Failed to initialize mock backend");
    assert!(mock_backend.is_initialized(), "Backend should be initialized");
    assert!(mock_backend.has_operation("initialize"), "Initialize operation should be logged");
    
    // Test configuration
    mock_backend.set_config("test_key", "test_value");
    assert_eq!(mock_backend.get_config("test_key"), Some("test_value".to_string()));
    
    // Test shutdown
    mock_backend.shutdown().expect("Failed to shutdown mock backend");
    assert!(!mock_backend.is_initialized(), "Backend should not be initialized after shutdown");
    assert!(mock_backend.has_operation("shutdown"), "Shutdown operation should be logged");
...
fn test_ebpf_map_operations() {
    ...
    if let Ok(mut bpf) = Ebpf::load_file(ebpf_path) {
        ...
        if let Some(map) = bpf.map_mut("ALLOWED_GATEWAYS") {
            ...
            gateways.insert(key, 1, 0).expect("Failed to insert gateway");
...
fn test_macos_backend_integration() {
    ...
    backend.initialize().expect("Failed to initialize macOS backend");
    ...
    backend.shutdown().expect("Failed to shutdown macOS backend");
...
fn test_windows_backend_integration() {
    ...
    backend.initialize().expect("Failed to initialize Windows backend");
    ...
    backend.shutdown().expect("Failed to shutdown Windows backend");
...
fn test_backend_error_handling() {
    ...
    assert!(!is_initialized_before_init, "Backend should not be initialized initially");
...
fn test_backend_performance() {
    ...
    // Verify performance
    assert!(elapsed < Duration::from_secs(1), "Operations should complete within 1 second");
    
    let operation_count = backend.get_operations().len();
    assert_eq!(operation_count, 1000, "Should have 1000 operations logged");
    
    println!("✓ Backend performance tests passed (1000 operations in {:?}", elapsed);
...
fn test_backend_concurrent_operations() {
    ...
    let backend = Arc::new(MockBackend::new("concurrent_test", "mock", "cross-platform"));
    backend.initialize().expect("Failed to initialize backend");
    ...
    for thread_id in 0..10 {
        let thread_operations = operations.iter()
            .filter(|op| op.starts_with(&format!("thread_{}_", thread_id)))
            .count();
        assert_eq!(thread_operations, 100, "Thread {} should have 100 operations", thread_id);
    }
