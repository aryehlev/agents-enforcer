...
fn test_event_type_filtering() {
    ...
    let network_handler =
        MockEventHandler::new("network_handler").with_filter(|event| event.event_type == "network");
...
fn test_custom_event_filtering() {
    ...
    let complex_handler = MockEventHandler::new("complex_handler").with_filter(|event| {
...
fn test_custom_fields() {
    ...
    // Verify metadata
    assert_eq!(
        event.metadata.get("destination_port"),
        Some(&"443".to_string())
    );
    assert_eq!(event.metadata.get("protocol"), Some(&"HTTPS".to_string()));
    assert_eq!(
        event.metadata.get("bytes_transferred"),
        Some(&"1024".to_string())
    );
...
fn test_event_performance() {
    ...
    assert!(
        ...
        "Performance too low: {:.2} events/sec",
        events_per_second
    );
...
fn test_concurrent_event_handling() {
    ...
    assert_eq!(stats.total_events, event_count);
    println!(
        "Processed {} events concurrently in {:?}",
        event_count, elapsed
    );
...
fn test_event_buffer_management() {
    ...
    println!(
        "Buffer processed {} events successfully",
        stats.total_events
    );
