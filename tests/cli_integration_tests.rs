...
fn test_cli_config_loading() {
    ...
    let config_path = config_generator.generate_dev_yaml()
    ...
    loader.load_config(&config_path).expect("Failed to load config");
    ...
    let non_existent_path = temp_manager.create_temp_dir()
...
fn test_cli_config_validation() {
    ...
    // Test valid config
    let valid_config_path = config_generator.generate_minimal_yaml()
    ...
    impl ConfigValidator {
        ...
        fn validate(&mut self, config_path: &PathBuf) -> Result<(), anyhow::Error> {
            ...
            if !content.contains("logging:") {
                self.warnings.push("No logging configuration found".to_string());
            }
...
fn test_cli_environment_variables() {
    ...
    assert_eq!(env_config.config_file, Some("/tmp/test_config.yaml".to_string()));
...
fn test_cli_backend_commands() {
    ...
    let commands = vec![
        ...
        ("agent-gateway-enforcer backend status", BackendCommand::Status { backend_name: None }),
        ("agent-gateway-enforcer backend status ebpf-linux", BackendCommand::Status { backend_name: Some("ebpf-linux".to_string()) }),
        ("agent-gateway-enforcer backend start mock", BackendCommand::Start { backend_name: "mock".to_string() }),
        ("agent-gateway-enforcer backend stop mock", BackendCommand::Stop { backend_name: "mock".to_string() }),
        ("agent-gateway-enforcer backend restart ebpf-linux", BackendCommand::Restart { backend_name: "ebpf-linux".to_string() }),
        ("agent-gateway-enforcer backend switch macos-desktop", BackendCommand::Switch { backend_name: "macos-desktop".to_string() }),
    ];
...
fn test_cli_backend_lifecycle() {
    ...
    impl MockBackendManager {
        fn new() -> Self {
            ...
            backends.insert("mock".to_string(), MockBackendStatus {
                name: "mock".to_string(),
            ...
            backends.insert("ebpf-linux".to_string(), MockBackendStatus {
...
fn test_cli_monitoring_commands() {
    ...
    let commands = vec![
        ("agent-gateway-enforcer monitor --mode realtime", MonitorCommand {
        ...
        ("agent-gateway-enforcer monitor --mode export --format json --output metrics.json", MonitorCommand {
        ...
        ("agent-gateway-enforcer monitor --filter severity=error --filter type=network", MonitorCommand {
            mode: "log".to_string(),
...
fn test_cli_invalid_arguments() {
    ...
    let error_cases = vec![
        ("agent-gateway-enforcer --invalid-flag", "unrecognized flag '--invalid-flag'"),
        ("agent-gateway-enforcer start --config", "missing argument for '--config'"),
        ("agent-gateway-enforcer start --port abc", "invalid value 'abc' for '--port'"),
        ("agent-gateway-enforcer nonexistent-command", "unrecognized subcommand 'nonexistent-command'"),
        ("agent-gateway-enforcer backend start", "missing required argument 'BACKEND_NAME'"),
    ];
    
    for (command, expected_error) in error_cases {
        // In a real implementation, this would execute the command
        // For now, we'll verify the error pattern
        assert!(expected_error.contains("unrecognized") || 
...
fn test_cli_permission_errors() {
    ...
    for error in permission_errors {
        assert!(error.contains("Permission") || error.contains("privileges") || error.contains("permitted"));
    }
