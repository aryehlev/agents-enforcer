...
fn test_yaml_configuration_loading() {
    ...
    let config_path = config_generator.generate_minimal_yaml().expect("Failed to generate minimal YAML");
    assert!(config_path.exists(), "Minimal YAML config should exist");
    
    // Test development YAML config
    let dev_config_path = config_generator.generate_dev_yaml().expect("Failed to generate dev YAML");
    assert!(dev_config_path.exists(), "Development YAML config should exist");
    
    // Test production YAML config
    let prod_config_path = config_generator.generate_prod_yaml().expect("Failed to generate prod YAML");
    assert!(prod_config_path.exists(), "Production YAML config should exist");
    ...
    assert!(yaml_content.contains("server:"), "YAML should contain server section");
    assert!(yaml_content.contains("backend:"), "YAML should contain backend section");
    assert!(yaml_content.contains("logging:"), "YAML should contain logging section");
...
fn test_json_configuration_loading() {
    ...
    let config_path = config_generator.generate_json().expect("Failed to generate JSON config");
    ...
    let parsed: serde_json::Value = serde_json::from_str(&json_content)
        .expect("JSON config should be valid JSON");
    
    // Verify structure
    assert!(parsed.get("server").is_some(), "JSON should contain server section");
    assert!(parsed.get("backend").is_some(), "JSON should contain backend section");
    assert!(parsed.get("logging").is_some(), "JSON should contain logging section");
...
fn test_toml_configuration_loading() {
    ...
    let config_path = config_generator.generate_toml().expect("Failed to generate TOML config");
    ...
    assert!(toml_content.contains("[server]"), "TOML should contain server section");
    assert!(toml_content.contains("[backend]"), "TOML should contain backend section");
    assert!(toml_content.contains("[logging]"), "TOML should contain logging section");
    ...
    for (config_type, result) in valid_configs {
        match result {
            Ok(config_path) => {
                ...
                assert!(!content.trim().is_empty(), "{} config should not be empty", config_type);
...
fn test_invalid_configuration_handling() {
    ...
    let invalid_config_path = config_generator.generate_invalid_config()
        .expect("Failed to generate invalid config");
    
    assert!(invalid_config_path.exists(), "Invalid config should exist");
    
    let invalid_content = std::fs::read_to_string(&invalid_config_path)
        .expect("Failed to read invalid config");
    
    // Should contain intentionally invalid content
    assert!(invalid_content.contains("invalid_port_number"), "Should contain invalid port");
...
fn test_configuration_templates() {
    ...
    let minimal_config = config_generator.generate_minimal_yaml().expect("Failed to generate minimal config");
    let minimal_content = std::fs::read_to_string(&minimal_config).expect("Failed to read minimal config");
    ...
    let dev_config = config_generator.generate_dev_yaml().expect("Failed to generate dev config");
    ...
    let prod_config = config_generator.generate_prod_yaml().expect("Failed to generate prod config");
...
"#;
    ...
    let config_path = temp_manager.create_temp_file("test_config.yaml", test_config)
        ...
        fn load_config(&mut self) -> Result<(), anyhow::Error> {
            ...
            for line in content.lines() {
                ...
                if trimmed.starts_with("host:") {
                    self.config.insert("server.host".to_string(), 
                        trimmed.split(':').nth(1).unwrap_or("").trim().trim_matches('"').to_string());
                } else if trimmed.starts_with("port:") {
                    self.config.insert("server.port".to_string(), 
                    ...
                    self.config.insert("backend.type".to_string(), 
                        trimmed.split(':').nth(1).unwrap_or("").trim().trim_matches('"').to_string());
                } else if trimmed.starts_with("level:") {
                    self.config.insert("logging.level".to_string(), 
                        trimmed.split(':').nth(1).unwrap_or("").trim().trim_matches('"').to_string());
                }
            }
    ...
    assert_eq!(config_manager.get("server.host"), Some(&"127.0.0.1".to_string()));
    assert_eq!(config_manager.get("server.port"), Some(&"8080".to_string()));
    assert_eq!(config_manager.get("backend.type"), Some(&"mock".to_string()));
    assert_eq!(config_manager.get("logging.level"), Some(&"info".to_string()));
...
"#;
    ...
    let config_path = temp_manager.create_temp_file("watchable_config.yaml", initial_config)
    ...
    assert!(!watcher.check_for_changes().expect("Failed to check for changes"), 
...
"#;
    ...
    let config_path = temp_manager.create_temp_file("hot_reload_config.yaml", initial_config)
        .expect("Failed to create initial config");
        ...
        fn reload_config(&mut self) -> Result<Vec<String>, anyhow::Error> {
            ...
            for line in content.lines() {
                ...
                if trimmed.starts_with("port:") {
                    ...
                    let value = trimmed.split(':').nth(1).unwrap_or("").trim().trim_matches('"').to_string();
            ...
            for (key, new_value) in &new_config {
                if let Some(old_value) = self.current_config.get(key) {
                    if old_value != new_value {
                        changes.push(format!("{} changed from {} to {}", key, old_value, new_value));
                    }
                } else {
    ...
    assert!(initial_changes.len() >= 3, "Should load initial configuration values");
...
"#;
    ...
    assert!(changes.iter().any(|c| c.contains("server.port")), "Should detect port change");
    assert!(changes.iter().any(|c| c.contains("server.workers")), "Should detect workers change");
...
fn test_missing_configuration_files() {
    ...
    let temp_manager = TempDirManager::new();
    let missing_config_path = temp_manager.create_temp_dir()
        .expect("Failed to create temp dir")
        .join("nonexistent_config.yaml");
    
    // Test handling of missing configuration files
    assert!(!missing_config_path.exists(), "Configuration file should not exist");
...
"#;
    ...
    let malformed_path = temp_manager.create_temp_file("malformed.yaml", malformed_yaml)
...
"#;
    ...
    let config_path = temp_manager.create_temp_file("permission_test.yaml", config_content)
