            ...
            BackendType::Auto => {
                ...
                if !platform.is_supported() {
                    return Err(anyhow::anyhow!(
                        "Auto-detection not supported on platform: {:?}", platform
                    ));
                }
            }
    ...
    fn validate(&self, config: &UnifiedConfig) -> Result<()> {
        for (index, gateway) in config.gateways.iter().enumerate() {
            ...
            if gateway.address.is_empty() {
                return Err(anyhow::anyhow!(
                    "Gateway {} has empty address", index
                ));
            }
            ...
            if gateway.protocols.is_empty() {
                return Err(anyhow::anyhow!(
                    "Gateway {} has no protocols specified", index
                ));
            }
            ...
            if let Ok(addr) = gateway.address.parse::<SocketAddr>() {
                if addr.port() < 1 || addr.port() > 65535 {
                    return Err(anyhow::anyhow!(
                        "Gateway {} has invalid port: {}", index, addr.port()
                    ));
                }
            }
    ...
    fn validate(&self, config: &UnifiedConfig) -> Result<()> {
        for (index, rule) in config.file_access.rules.iter().enumerate() {
            ...
            if rule.path.is_empty() {
                return Err(anyhow::anyhow!(
                    "File access rule {} has empty path", index
                ));
            }
            ...
            if rule.permissions.is_empty() {
                return Err(anyhow::anyhow!(
                    "File access rule {} has no permissions specified", index
                ));
            }
            ...
            if rule.applies_to.is_empty() {
                return Err(anyhow::anyhow!(
                    "File access rule {} has no target agents specified", index
                ));
            }
        ...
        for (index, path_pattern) in config.file_access.protected_paths.iter().enumerate() {
            if path_pattern.pattern.is_empty() {
                return Err(anyhow::anyhow!(
                    "Protected path {} has empty pattern", index
                ));
            }
...
impl FileAccessValidator {
    fn validate_condition(&self, condition: &RuleCondition, rule_index: usize, cond_index: usize) -> Result<()> {
        ...
        if condition.value.is_empty() {
            return Err(anyhow::anyhow!(
                "File access rule {} condition {} has empty value", rule_index, cond_index
            ));
        }
        ...
        match &condition.condition_type {
            ConditionType::ProcessName => {
                ...
                if !condition.value.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_' || c == '.') {
                    return Err(anyhow::anyhow!(
...
impl ConfigValidator for MetricsValidator {
    fn validate(&self, config: &UnifiedConfig) -> Result<()> {
        ...
        if config.metrics.port < 1 || config.metrics.port > 65535 {
            return Err(anyhow::anyhow!(
                "Metrics port {} is out of valid range (1-65535)", config.metrics.port
            ));
        }
        ...
        if !config.metrics.endpoint.starts_with('/') {
            return Err(anyhow::anyhow!(
                "Metrics endpoint '{}' must start with '/'", config.metrics.endpoint
            ));
        }
    ...
    fn validate(&self, config: &UnifiedConfig) -> Result<()> {
        ...
        if let Some(file_path) = &config.logging.file {
            if file_path.as_os_str().is_empty() {
                return Err(anyhow::anyhow!(
                    "Logging file path cannot be empty"
                ));
            }
            ...
            if let Some(parent) = file_path.parent() {
                if !parent.as_os_str().is_empty() && !parent.exists() {
                    return Err(anyhow::anyhow!(
                        ...
                        parent.display(), file_path.display()
                    ));
                }
            }
        }
        ...
        for (field_name, log_field) in &config.logging.fields {
            if field_name.is_empty() {
                return Err(anyhow::anyhow!(
                    "Log field name cannot be empty"
                ));
            }
            ...
            if let Some(format) = &log_field.format {
                if format.is_empty() {
                    return Err(anyhow::anyhow!(
                        "Log field '{}' format cannot be empty", field_name
                    ));
                }
            }
        }
    ...
    fn validate(&self, config: &UnifiedConfig) -> Result<()> {
        ...
        if config.ui.web_dashboard.enabled {
            ...
            if config.ui.web_dashboard.host.is_empty() {
                return Err(anyhow::anyhow!(
                    "Web dashboard host cannot be empty"
                ));
            }
            
            // Validate TLS configuration
            if config.ui.web_dashboard.tls {
                if config.ui.web_dashboard.cert_file.is_none() || config.ui.web_dashboard.key_file.is_none() {
                    return Err(anyhow::anyhow!(
                        "Web dashboard TLS is enabled but cert_file or key_file is not specified"
                    ));
                }
                
                if let (Some(cert_file), Some(key_file)) = (&config.ui.web_dashboard.cert_file, &config.ui.web_dashboard.key_file) {
                    if !cert_file.exists() {
                        return Err(anyhow::anyhow!(
