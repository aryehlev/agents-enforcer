...
impl ConfigMigrator {
    ...
    pub async fn migrate_from_legacy(&self, legacy_path: &Path) -> Result<UnifiedConfig> {
        let legacy_content = tokio::fs::read_to_string(legacy_path).await.map_err(|e| {
            anyhow::anyhow!(
        ...
        Err(anyhow::anyhow!(
            "Unable to detect legacy configuration format"
        ))
    }
    ...
    pub fn validate_migrated_config(&self, config: &UnifiedConfig) -> Result<()> {
        ...
        for (index, gateway) in config.gateways.iter().enumerate() {
            if gateway.address.is_empty() {
                return Err(anyhow::anyhow!(
                    "Gateway {} has empty address after migration",
                    index
                ));
            }
        }
        ...
        for (index, rule) in config.file_access.rules.iter().enumerate() {
            if rule.path.is_empty() {
                return Err(anyhow::anyhow!(
                    "File access rule {} has empty path after migration",
                    index
                ));
            }
        }
    ...
    fn convert_ebpf_file_rule(&self, rule_value: &Value) -> Result<FileAccessRule> {
        ...
        let rule_type_str = rule_value
            .get("rule_type")
            .and_then(|rt| rt.as_str())
            .unwrap_or("allow");

        let permissions_value = rule_value
            .get("permissions")
            .and_then(|p| p.as_u64())
            .unwrap_or(0) as u8;

        let is_prefix = rule_value
            .get("is_prefix")
        ...
        if permissions_value & 1 != 0 {
            permissions.push(FilePermission::Read);
        }
        if permissions_value & 2 != 0 {
            permissions.push(FilePermission::Write);
        }
        if permissions_value & 4 != 0 {
            permissions.push(FilePermission::Execute);
        }
        if permissions_value & 8 != 0 {
            permissions.push(FilePermission::Delete);
        }
...
impl ReportingConfigMigrator {
    ...
    pub async fn migrate_from_legacy(
        &mut self,
        legacy_path: &Path,
    ) -> Result<(UnifiedConfig, MigrationReport)> {
    ...
    fn generate_migration_report(&mut self, config: &UnifiedConfig) {
        self.report
            .add_change("Migrated to unified configuration format");

        if !config.gateways.is_empty() {
            self.report.add_change(&format!(
                "Migrated {} gateway configurations",
                config.gateways.len()
            ));
        }
        ...
        if config.file_access.enabled {
            self.report
                .add_change("Migrated file access control settings");
        }
        ...
        if config.gateways.is_empty() {
            self.report
                .add_warning("No gateways configured - network access may be blocked");
        }

        if config.file_access.enabled && config.file_access.rules.is_empty() {
            self.report
                .add_warning("File access enabled but no rules configured");
        }
    }
...
mod tests {
    ...
    async fn test_ebpf_config_migration() {
        ...
        let config = migrator
            .migrate_from_ebpf_config(eBPF_config)
            .await
            .unwrap();
