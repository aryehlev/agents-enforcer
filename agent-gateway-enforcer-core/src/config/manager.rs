...
impl ConfigManager {
    ...
    pub async fn load(&mut self) -> Result<UnifiedConfig> {
        ...
        *current = config.clone();
        ...
        // Notify watchers
        self.notify_watchers(ConfigEvent::Loaded(config.clone()))
            .await;
    ...
    pub async fn save(&self, config: &UnifiedConfig) -> Result<()> {
        ...
        *current = config.clone();
        ...
        // Notify watchers
        self.notify_watchers(ConfigEvent::Saved(config.clone()))
            .await;
                        ...
                        if matches!(event.kind, EventKind::Modify(_)) {
                            ...
                            match Self::reload_config(&config_path).await {
                                Ok(new_config) => {
                                    ...
                                    *current = new_config.clone();
                                    ...
                                    for watcher in watchers.iter_mut() {
                                        let _ = watcher
                                            .on_config_changed(ConfigEvent::HotReloaded(
                                                new_config.clone(),
                                            ))
                                            .await;
                                    }
                                ...
                                Err(e) => {
                                    ...
                                    for watcher in watchers.iter_mut() {
                                        let _ = watcher
                                            .on_config_changed(ConfigEvent::ValidationError(
                                                e.to_string(),
                                            ))
                                            .await;
                                    }
    ...
    fn parse_config(&self, content: &str) -> Result<UnifiedConfig> {
        ...
        Err(anyhow::anyhow!(
            "Unable to determine configuration format. Supported formats: YAML, TOML, JSON"
        ))
    }
    ...
    async fn apply_env_overrides(&self, config: &mut UnifiedConfig) -> Result<()> {
        ...
        for (key, value) in gateway_envs {
            ...
            if parts.len() >= 4 {
                ...
                match parts[2] {
                    ...
                    "ENABLED" => {
                        config.gateways[index].enabled = value.parse().map_err(|e| {
                            anyhow::anyhow!("Invalid gateway enabled flag {}: {}", value, e)
                        })?
                    }
                    "PRIORITY" => {
                        config.gateways[index].priority = value.parse().map_err(|e| {
                            anyhow::anyhow!("Invalid gateway priority {}: {}", value, e)
                        })?
                    }
                    _ => {}
                }
        ...
        if let Ok(enabled) = std::env::var("AGENT_GATEWAY_FILE_ACCESS_ENABLED") {
            config.file_access.enabled = enabled.parse().map_err(|e| {
                anyhow::anyhow!("Invalid file access enabled flag {}: {}", enabled, e)
            })?;
        }
        ...
        if let Ok(enabled) = std::env::var("AGENT_GATEWAY_WEB_DASHBOARD_ENABLED") {
            config.ui.web_dashboard.enabled = enabled.parse().map_err(|e| {
                anyhow::anyhow!("Invalid web dashboard enabled flag {}: {}", enabled, e)
            })?;
        }

        if let Ok(port) = std::env::var("AGENT_GATEWAY_WEB_DASHBOARD_PORT") {
            config.ui.web_dashboard.port = port
                .parse()
...
impl ConfigWatcher for LoggingConfigWatcher {
    async fn on_config_changed(&mut self, event: ConfigEvent) {
        match event {
            ...
            ConfigEvent::ValidationError(ref e) => {
