...
use agent_gateway_enforcer_core::backend::{BackendRegistry, BackendType, UnifiedConfig, GatewayConfig, EnforcementBackend};
use agent_gateway_enforcer_backend_ebpf_linux::{EbpfLinuxBackend, registry};
...
async fn test_backend_lifecycle() {
    ...
    assert_eq!(health.status, agent_gateway_enforcer_core::backend::HealthStatus::Unhealthy);
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if result.is_ok() {
            ...
            let health = backend.health_check().await.unwrap();
            assert_eq!(health.status, agent_gateway_enforcer_core::backend::HealthStatus::Degraded);
        }
    }
...
async fn test_backend_factory() {
    ...
    assert_eq!(info.platform, agent_gateway_enforcer_core::backend::Platform::Linux);
...
fn test_backend_creation() {
    let backend = EbpfLinuxBackend::new();
    assert_eq!(backend.backend_type(), BackendType::EbpfLinux);
    assert_eq!(backend.platform(), agent_gateway_enforcer_core::backend::Platform::Linux);
}